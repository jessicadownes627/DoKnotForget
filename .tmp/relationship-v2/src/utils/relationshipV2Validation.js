...
function validateExpectation(fixtureName, caseLabel, actual, expected) {
    ...
    if (expected.directRoleToUser !== undefined) {
        assertEqual(`${fixtureName} / ${caseLabel} / directRoleToUser`, actual.directLinkToUser?.relationshipToAnchor ?? null, expected.directRoleToUser);
    }
    ...
    if (expected.anchorPersonId !== undefined) {
        assertEqual(`${fixtureName} / ${caseLabel} / anchorPersonId`, actual.primaryLink?.anchor.kind === "person" ? actual.primaryLink.anchor.personId : null, expected.anchorPersonId);
    }
    if (expected.recipientPersonId !== undefined) {
        assertEqual(`${fixtureName} / ${caseLabel} / recipientPersonId`, actual.suggestedRecipient?.ref.kind === "person" ? actual.suggestedRecipient.ref.personId : null, expected.recipientPersonId);
    }
...
export function runRelationshipV2Validation() {
    ...
    for (const fixture of relationshipV2Fixtures) {
        ...
        for (const testCase of fixture.cases) {
            ...
            lines.push(`  PASS ${testCase.label} -> primary=${resolved.primaryLink?.relationshipToAnchor ?? "none"} recipient=${describeRelationshipV2Ref(resolved.suggestedRecipient?.ref ?? { kind: "unknown" })}`);
        }
    }
    return lines.join("\n");
}
