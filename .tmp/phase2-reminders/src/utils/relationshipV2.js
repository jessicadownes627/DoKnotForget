...
const USER_ANCHOR = { kind: "user" };
function personRef(personId) {
    return { kind: "person", personId };
}
function childRef(parentId, childId) {
    return { kind: "child", parentId, childId };
}
function personAnchor(personId) {
    return { kind: "person", personId };
}
...
function relationshipSourceRank(source) {
    switch (source) {
        case "persistedV2":
            return 6;
...
function linkSortValue(link) {
    return (confidenceRank(link.confidence) * 100 +
        roleRank(link.relationshipToAnchor) * 10 +
        relationshipSourceRank(link.source));
}
function reverseRelationship(role) {
    switch (role) {
        case "child":
            return "parent";
        case "parent":
...
function pushUniqueLink(target, seen, link) {
...
function findChild(people, parentId, childId) {
    const parent = findPerson(people, parentId);
    if (!parent)
        return null;
    return (parent.children ?? []).find((child) => child.id === childId) ?? null;
}
function firstAvailableName(value, fallback) {
    const trimmed = (value ?? "").trim();
    return trimmed || fallback;
}
export function buildRelationshipV2Links({ people, relationships, persistedLinks = [], }) {
    const links = [];
    const seen = new Set();
    ...
    for (const relationship of normalized) {
        if (relationship.type === "child") {
            pushUniqueLink(links, seen, buildLink({
    ...
    for (const person of people) {
        ...
        if (subjectsWithAnyLink.has(key))
            continue;
        pushUniqueLink(links, seen, buildLink({
            subject: personRef(person.id),
            anchor: USER_ANCHOR,
...
export function normalizeRelationshipV2Links(links) {
    const seen = new Set();
    const normalized = [];
...
export function resolveRelationshipV2Entity(subject, people) {
...
function resolveRecipientFromLink(subject, primaryLink, people) {
    if (!primaryLink) {
...
function sortLinksForResolution(a, b) {
...
export function resolveRelationshipV2Context(args) {
    ...
    const directLinkToUser = allLinks.find((link) => link.anchor.kind === "user" && link.relationshipToAnchor !== "unknown") ??
        allLinks.find((link) => link.anchor.kind === "user") ??
        null;
    const anchoredLink = allLinks.find((link) => link.anchor.kind === "person") ?? null;
    const primaryLink = directLinkToUser && directLinkToUser.relationshipToAnchor !== "unknown"
        ? directLinkToUser
...
export function describeRelationshipV2Ref(ref) {
    if (ref.kind === "unknown")
        return "unknown";
    if (ref.kind === "person")
        return `person:${ref.personId}`;
