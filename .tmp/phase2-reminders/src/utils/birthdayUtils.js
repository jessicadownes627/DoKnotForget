...
function parseIsoMonthDay(value) {
...
function calculateDaysUntil(dateString, baseDate = new Date()) {
    ...
    if (nextOccurrence.getMonth() !== parts.month - 1 || nextOccurrence.getDate() !== parts.day)
        return null;
    if (nextOccurrence.getMonth() < base.getMonth() ||
...
export function daysUntilDate(target, baseDate = new Date()) {
    const base = startOfDay(baseDate);
    const t = startOfDay(target);
    const msPerDay = 24 * 60 * 60 * 1000;
    return Math.round((t.getTime() - base.getTime()) / msPerDay);
}
export function getNextBirthdayFromIso(birthdayIso, baseDate = new Date()) {
...
export function getNextChildBirthday(birthday) {
    const base = startOfDay(new Date());
    const month = birthday.getMonth();
    const day = birthday.getDate();
    const thisYear = new Date(base.getFullYear(), month, day);
    if (thisYear.getMonth() !== month || thisYear.getDate() !== day)
        return thisYear;
