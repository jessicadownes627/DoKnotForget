...
function joinRecipientFirstNames(recipients) {
    return recipients.map((recipient) => contactFirstName(recipient.name)).join(" & ");
}
export function reminderRelativeLabel(reminderType) {
...
export function reminderEventDate(reminder) {
...
function calculateAge(birthday, referenceDate = new Date()) {
    ...
    const hasHadBirthdayThisYear = today.getMonth() + 1 > month ||
        (today.getMonth() + 1 === month && today.getDate() >= day);
    if (!hasHadBirthdayThisYear)
        age -= 1;
    return age >= 0 ? age : undefined;
}
function resolveChildParentContacts(person, people, parents) {
...
function getChildBirthdayContext(reminder, people, today) {
    ...
    const child = person.children?.find((candidate) => {
...
function mapPersonToRecipient(person) {
...
function resolveRecipientsFromAnchoredChildLinks(people, reminderRelationshipContext) {
    return reminderRelationshipContext.allLinks
        .flatMap((link) => {
        const anchor = link.anchor;
        if (anchor.kind !== "person" || link.relationshipToAnchor !== "child")
            return [];
        const recipient = mapPersonToRecipient(people.find((candidate) => candidate.id === anchor.personId) ?? null);
        return recipient ? [recipient] : [];
    })
        .filter((recipient) => Boolean(recipient));
}
export function resolveReminderContext(reminder, people, relationships, today, relationshipV2Links = buildRelationshipV2Links({ people, relationships })) {
    const person = people.find((candidate) => candidate.id === reminder.personId) ?? null;
    ...
    if (reminder.momentType === "childBirthday") {
        ...
        return {
            ...
            recipients,
            actionHeading: recipients.length > 0
    ...
    if (reminder.momentType === "anniversary") {
        ...
        const anniversaryRelationshipContext = person
            ...
            : null;
        const partnerLink = anniversaryRelationshipContext?.allLinks.find((link) => link.anchor.kind === "person" && link.relationshipToAnchor === "partner") ?? null;
        const partnerAnchor = partnerLink?.anchor;
        const partner = partnerAnchor?.kind === "person"
...
export function buildResolvedReminderLabel(reminder, people, relationships, today, relationshipV2Links = buildRelationshipV2Links({ people, relationships })) {
    const person = people.find((candidate) => candidate.id === reminder.personId) ?? null;
